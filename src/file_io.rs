...
pub fn Open<'a>(
  ...
  let open_status: &UseState<Option<Result<mdf::Mdf, serde_json::Error>>> = use_state(cx, || None);
  ...
  cx.render(rsx!{
    a { 
      ...
      onclick: open_file,
      i { 
        class:"fa-solid fa-folder-open mr-1"
      }, 
      "Open..." },
  })
}
...
pub fn SaveAs<'a>(
      ...
      async move {
        ...
        if let Some(file) = file {
          match std::fs::File::create(file.path()) {
            Ok(file) => {
              save_status.set(Some(serde_json::to_writer_pretty(std::io::BufWriter::new(file), &model_to_save).map_err(|e| format!("Error while writing file: {}", e))));
            },
            Err(errormsg) => {
              save_status.set(Some(Err(format!("Error while creating file: {}", errormsg))));
            }
          }
        }
      }
  ...
  cx.render(rsx!{
    a { 
      ...
      onclick: save_file,
      i { 
        class:"fa-solid fa-file-export mr-1"
      }, 
      "Save as..." },
  })
}
...
pub fn SaveAs<'a>(
  ...
  let save_file = move | _ | {
    ...
    match file_serialize {
      Ok(file_text) => {
        app_data.with_mut(|data| {
          data.web_file_save = Some(format!("data:text/plain;charset=utf-8,{}", js_sys::encode_uri_component(&file_text)));
        });
      },
      Err(message) => {
        app_data.with_mut(|data| {
