...
use super::tokenlist::{to_vhdl_token, TokenList};
...
impl GenModel {
    ...
    pub fn from_model(
        ...
        let top_name =
            token_list.generate_token(&templates.render(user_strings::GM_TOP_NAME, &context)?);
        let core_name =
            token_list.generate_token(&templates.render(user_strings::GM_CORE_NAME, &context)?);
        let core_instance =
            token_list.generate_token(&templates.render(user_strings::GM_CORE_INSTANCE, &context)?);
        let pkg_name =
            token_list.generate_token(&templates.render(user_strings::GM_PKG_NAME, &context)?);
        ...
        let interfaces = model
            ...
            .enumerate()
            .map(|(n, interface)| {
                GenInterface::from_interface(
                    interface,
                    PageType::Interface(n),
                    settings,
                    templates,
                    &token_name,
                    &single_interface,
...
impl GenInterface {
    ...
    pub fn from_interface(
        interface: &mdf::Interface,
        page: PageType,
        settings: &Settings,
        templates: &Tera,
        project_token_name: &String,
        single_interface: &bool,
        general_token_list: &mut TokenList,
    ) -> Result<Self, Box<dyn Error>> {
        ...
        let token_name = if *single_interface {
            "".to_owned()
        } else {
            to_vhdl_token(&name)
        };
        ...
        let address_width = match interface.get_address_width() {
            ...
            None => Err(GenError::new(
                &page,
                &format!("couldn't determine interface {} address width", name),
            ))?,
        };
        let data_width = match interface.get_data_width() {
            ...
            None => Err(GenError::new(
                &page,
                &format!("couldn't determine interface {} data width", name),
            ))?,
        };
        // go through all the registers and check if at least one uses an address stride
        let use_stride = interface.registers.iter().fold(false, |use_stride, reg| {
            use_stride || reg.address.stride.is_some()
        });
        let use_not_stride = interface
            .registers
            .iter()
        ...
        let pif_name = general_token_list
            .generate_token(&templates.render(user_strings::GI_PIF_NAME, &context)?);
        let pif_instance = general_token_list
            .generate_token(&templates.render(user_strings::GI_PIF_INSTANCE, &context)?);
        let core2pif_name = general_token_list
        ...
        let data_width_const_name = general_token_list
            .generate_token(&templates.render(user_strings::GI_DATA_WIDTH_CONST_NAME, &context)?);
        ...
        let ports_names: HashMap<String, String> = ports
            .iter()
            .map(|signal| (signal.function.clone(), signal.name.clone()))
            .collect();
        ...
        let registers = match page {
            PageType::Interface(int_num) => {
                ...
                interface
                    ...
                    .enumerate()
                    .map(|(n, register)| {
                        GenRegister::from_register(
                            register,
                            PageType::Register(int_num, n, None),
                            settings,
                            templates,
                            project_token_name,
                            &token_name,
                            data_width,
                            general_token_list,
                            &mut corfe2pif_token_list,
                            &mut pif2core_token_list,
                        )
                    })
            ...
            _ => Err(GenError::new(
                &page,
                "wrong value for the page parameter in register call",
            ))?,
        };

        // go through all registers to see if some have some doc details
        let regs_doc_details = registers
            .iter()
            .fold(false, |prev, reg| prev || reg.doc_details);
...
) -> Result<GenStructSignal, Box<dyn Error>> {
...
impl GenRegister {
    ...
    pub fn from_register(
        ...
        templates: &Tera,
        project_token_name: &String,
        interface_token_name: &String,
        interface_data_width: u32,
        general_token_list: &mut TokenList,
        corfe2pif_token_list: &mut TokenList,
        pif2core_token_list: &mut TokenList,
    ) -> Result<Self, Box<dyn Error>> {
        let name = register.name.clone();
        let token_name = to_vhdl_token(&name);
        let address_hex = format!(
            "{:x}",
            register.address.value.ok_or("address not defined")?.value
        );
        ...
        let fields = if !is_bitfield {
            ...
            let width_const_name = general_token_list
                .generate_token(&templates.render(user_strings::GR_WIDTH_CONST_NAME, &context)?);

            let rw_mode = register
                .access
                .ok_or(GenError::new(&page, "access type needed for register"))?;
            ...
            let is_in_core = register.location.ok_or(GenError::new(
                &page,
                "location for register {} needs to be specified",
            ))? == mdf::LocationType::Core;

            let core_read_enable = register
                .core_signal_properties
                .use_read_enable
                .unwrap_or(false);
            let core_write_enable = register
                .core_signal_properties
                .use_write_enable
                .unwrap_or(false);
            ...
            if core2pif_has_data {
                core2pif.push(gen_registersignal(
                    templates,
                    "data",
                    user_strings::GR_DATA_NAME,
                    &sig_type_complete,
                    user_strings::GR_DATA_DESCRIPTION,
                    &context,
                    corfe2pif_token_list,
                )?);
            }
            ...
            if core_read_enable {
                pif2core.push(gen_registersignal(
                    templates,
                    "read_enable",
                    user_strings::GR_READ_ENABLE_NAME,
                    "boolean",
                    user_strings::GR_READ_ENABLE_DESCRIPTION,
                    &context,
                    pif2core_token_list,
                )?);
            }
            if core_write_enable {
                pif2core.push(gen_registersignal(
                    templates,
                    "write_enable",
                    user_strings::GR_WRITE_ENABLE_NAME,
                    "boolean",
                    user_strings::GR_WRITE_ENABLE_DESCRIPTION,
                    &context,
                    pif2core_token_list,
                )?);
            }
            ...
            match page {
                PageType::Register(int_num, reg_num, None) => register
                    .fields
                    .iter()
                    .enumerate()
                    .map(|(n, field)| {
                        GenField::from_field(
                            register,
                            field,
                            PageType::Register(int_num, reg_num, Some(n)),
                            settings,
                            templates,
                            project_token_name,
                            interface_token_name,
                    ...
                    .collect::<Result<Vec<GenField>, Box<dyn Error>>>()?,
                _ => Err(GenError::new(
                    &page,
                    "wrong value for the page parameter in register call",
                ))?,
        ...
        let address_const_name = general_token_list
            .generate_token(&templates.render(user_strings::GR_ADDRESS_CONST_NAME, &context)?);
        let stride_count_const_name = general_token_list
            .generate_token(&templates.render(user_strings::GR_STRIDE_COUNT_CONST_NAME, &context)?);
...
impl GenField {
    ...
    pub fn from_field(
        ...
        let width = match field.position {
            ...
            mdf::FieldPosition::Field(msb, lsb) => {
                if msb >= lsb {
                    ...
                    Err(GenError::new(&page, "wrong bit order specified"))
                }?
            }
        };
        let width_matches_interface = width == interface_data_width;
        ...
        let width_const_name = general_token_list
            .generate_token(&templates.render(user_strings::GF_WIDTH_CONST_NAME, &context)?);
        let offset_const_name = general_token_list
            .generate_token(&templates.render(user_strings::GF_OFFSET_CONST_NAME, &context)?);
        ...
        let location = field
            .location
            .unwrap_or(register.location.ok_or(GenError::new(
                &page,
                "location needs to be defined for field or register",
            ))?);
        let is_in_core = location == mdf::LocationType::Core;

        let core_read_enable = field
            .core_signal_properties
            .use_read_enable
            .unwrap_or(false);
        let core_write_enable = field
            .core_signal_properties
            .use_write_enable
            .unwrap_or(false);
        ...
        if core2pif_has_data {
            core2pif.push(gen_registersignal(
                templates,
                "data",
                user_strings::GF_DATA_NAME,
                &sig_type_complete,
                user_strings::GF_DATA_DESCRIPTION,
                &context,
                corfe2pif_token_list,
            )?);
        }
