    ...
    egui::CentralPanel::default().show(ctx, |mut ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("New").clicked() {
                ...
                return_value = Some(page::PageType::Register(interface_num,interface.registers.len()-1));
                undo.register_modification("create new register", undo::ModificationType::Finished);
            }
        });

        let num_registers = interface.registers.len();
        let register_names : Vec<String> = interface.registers.iter().map(|int| int.name.clone()).collect();
                    ...
                    for (n, register_name) in register_names.iter().enumerate() {
                        ...
                        ui.horizontal(|ui| {
                            if ui.button("🗑").clicked() {
                                ...
                                undo.register_modification("delete register", undo::ModificationType::Finished);
                            }
                            ui.add_enabled_ui(n > 0, |ui| {
                                if ui.button("⬆").clicked() {
                                    interface.registers.swap(n-1,n);
                                    undo.register_modification("move register", undo::ModificationType::Finished);
                                }
                            });
