...
fn from_str() {
  let expected = vec![
    "first line", "second line", "third line"].iter().map(|x| x.to_string()).collect();
  ...
  assert_eq!(None,
    utils::textarea_to_opt_vec_str(&"".to_string()));
}
...
fn to_str() {
  let value : Vec<String>= vec![
    "line one", "line two", "line three"].iter().map(|x| x.to_string()).collect();

  assert_eq!(
    "".to_string(),
    utils::opt_vec_str_to_textarea(&None));
  assert_eq!(
    "line one".to_string(),
