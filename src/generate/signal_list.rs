...
pub fn to_gen_int_port(
    definition: &SignalDef<'_>,
    templates: &SignalTemplates<'_>,
    context: &tera::Context,
    general_token_list: &mut tokenlist::TokenList,
) -> Result<genmodel::GenIntPort, Box<dyn Error>> {
    ...
    let name = general_token_list.generate_token(&tera::Tera::one_off(
        &name_template,
        &new_context,
        false,
    )?);
...
pub fn to_port_list(
    ...
    context: &tera::Context,
    general_token_list: &mut tokenlist::TokenList,
) -> Result<Vec<genmodel::GenIntPort>, Box<dyn Error>> {
    ...
    defs.map(|x| to_gen_int_port(x, templates, &context, general_token_list))
        .collect()
}
